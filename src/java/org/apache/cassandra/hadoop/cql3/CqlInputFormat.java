...
public class CqlInputFormat extends org.apache.hadoop.mapreduce.InputFormat<Long, Row> implements org.apache.hadoop.mapred.InputFormat<Long, Row>
{
    ...
    private Map<TokenRange, Long> getSubSplits(String keyspace, String cfName, TokenRange range, Host host, Configuration conf, Session session) throws IOException
    {
    ...
    private Map<TokenRange, Long> describeSplits(String keyspace, String table, TokenRange tokenRange, Host host, int splitSize, int splitSizeMb, Session session)
    {
        ...
        // In 3.0 we rely on the below CQL query which is local and only computes estimates for the primary or local primary range; this
    ...
    private static ResultSet queryTableEstimates(Session session, Host host, String keyspace, String table, TokenRange tokenRange)
    {
        String query = String.format("SELECT mean_partition_size, partitions_count " +
                                     "FROM %s.%s " +
                                     "WHERE keyspace_name = ? AND table_name = ? AND range_type = 'local_primary' AND range_start = ? AND range_end = ?",
                                     SystemKeyspace.NAME,
        ...
        catch (InvalidQueryException e)
        {
            ...
            stmt = new SimpleStatement(query, keyspace, table, tokenRange.getStart().toString(), tokenRange.getEnd().toString()).setHost(host);
            return session.execute(stmt);
        }
    }

    private static Map<TokenRange, Long> splitTokenRange(TokenRange tokenRange, int splitCount, long splitPartitionCountEstimate)
    {
        List<TokenRange> splitRanges = tokenRange.splitEvenly(splitCount);
        Map<TokenRange, Long> rangesWithLength = new HashMap<>();
        for (TokenRange range : splitRanges)
            rangesWithLength.put(range, splitPartitionCountEstimate);
    ...
    class SplitFuture extends FutureTask<List<ColumnFamilySplit>>
    {
        private final SplitCallable splitCallable;

        public SplitFuture(SplitCallable splitCallable)
        {
            super(splitCallable);
