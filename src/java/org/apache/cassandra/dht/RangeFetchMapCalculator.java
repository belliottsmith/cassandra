...
public class RangeFetchMapCalculator
{
    ...
    static boolean isTrivial(Range<Token> range)
    {
        ...
        {
            l = BigInteger.valueOf(((Murmur3Partitioner.LongToken)range.left).token);
            r = BigInteger.valueOf(((Murmur3Partitioner.LongToken)range.right).token);
        }
        else if (partitioner instanceof RandomPartitioner)
        {
            l = ((RandomPartitioner.BigIntegerToken)range.left).token;
            r = ((RandomPartitioner.BigIntegerToken)range.right).token;
        }
        else
            return false;
