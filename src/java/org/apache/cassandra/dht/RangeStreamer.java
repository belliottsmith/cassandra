            ...
            for (Range<Token> range : rangeAddresses.keySet())
            {
                if (range.contains(toFetch.range()))
                {
                    ...
                    if (useStrictConsistency)
                    {
                        ...
                        {
                            // TODO: TR-Review is this removal safe? are we guaranteed to be fully replicating the range?
                            ...
                            // TODO: TR-Review are we guaranteed there will only be one endpoint? Seemingly we're aiming to get the node we're replacing, but it isn't obvious that we definitely do.
                        ...
                        {
                            // TODO: TR-Review why are we requiring that our sourceFilters all apply in this case?  Shouldn't this drop down to the non-strict case below?
                        ...
                        if (!all(sources, testSourceFilters))
                        {
                            throw new IllegalStateException("Necessary replicas for strict consistency were removed by source filters: " + sources.filter(not(testSourceFilters)));
                        }
                    }
                    else
                    {
