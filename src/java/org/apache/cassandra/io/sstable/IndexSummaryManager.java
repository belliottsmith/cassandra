...
public class IndexSummaryManager implements IndexSummaryManagerMBean
{
     ...
     * compacting.
     */
    private Pair<List<SSTableReader>, Map<UUID, Transaction>> getCompactingAndNonCompactingSSTables()
    {
        List<SSTableReader> allCompacting = new ArrayList<>();
        Map<UUID, Transaction> allNonCompacting = new HashMap<>();
        for (Keyspace ks : Keyspace.all())
        {
            for (ColumnFamilyStore cfStore: ks.getColumnFamilyStores())
            {
                ...
                do
                {
                    View view = cfStore.getDataTracker().getView();
                    allSSTables = view.sstables;
                    nonCompacting = ImmutableSet.copyOf(view.getUncompacting(allSSTables));
                }
                while (null == (txn = cfStore.getDataTracker().tryModify(nonCompacting, OperationType.UNKNOWN)));
    ...
    {
        Pair<List<SSTableReader>, Map<UUID, Transaction>> compactingAndNonCompacting = getCompactingAndNonCompactingSSTables();
        try
        {
        ...
        {
            for (Transaction modifier : compactingAndNonCompacting.right.values())
                modifier.close();
        }
    }
     ...
     */
    @VisibleForTesting
    public static List<SSTableReader> redistributeSummaries(List<SSTableReader> compacting, Map<UUID, Transaction> transactions, long memoryPoolBytes) throws IOException
    {
    ...
    private static List<SSTableReader> adjustSamplingLevels(List<SSTableReader> sstables, Map<UUID, Transaction> transactions,
