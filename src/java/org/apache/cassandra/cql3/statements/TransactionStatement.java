...
public class TransactionStatement implements CQLStatement.CompositeCQLStatement, CQLStatement.ReturningCQLStatement
{
    ...
    private ConsistencyLevel consistencyLevelForAccordRead(ClusterMetadata cm, TableMetadatas tables, Keys keys, @Nullable ConsistencyLevel consistencyLevel)
    {
    ...
    private ConsistencyLevel consistencyLevelForAccordRead(ClusterMetadata cm, TableMetadatas tables, Key key, ConsistencyLevel consistencyLevel)
    {
        // Null means no specific consistency behavior is required from Accord, it's functionally similar to
        // reading at ONE if you are reading data that wasn't written via Accord
        if (consistencyLevel == null)
        ...
        TableParams tableParams = tables.get(tableId).params;
    ...
    private static ConsistencyLevel consistencyLevelForAccordCommit(ClusterMetadata cm, TableMetadatas tables, TableMetadatasAndKeys.KeyCollector keys, @Nullable ConsistencyLevel consistencyLevel)
    {
    ...
    private static ConsistencyLevel consistencyLevelForAccordCommit(ClusterMetadata cm, TableMetadatas tables, Key key, @Nullable ConsistencyLevel consistencyLevel)
    {
        // Null means no specific consistency behavior is required from Accord, it's functionally similar to ANY
        // if you aren't reading the result back via Accord
        if (consistencyLevel == null)
    ...
    public Txn createTxn(ClientState state, QueryOptions options)
    {
        ...
        TableMetadatas tables = collectTables();
        ...
        if (updates.isEmpty())
        {
            ...
            Txn.Kind kind = shouldReadEphemerally(keys, tables.get((TableId)keys.get(0).prefix()).params, Read);
