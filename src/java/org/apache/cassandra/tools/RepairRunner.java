...
public class RepairRunner extends JMXNotificationProgressListener
{
    ...
    public void run() throws Exception
    {
        ...
        {
            while (!condition.await(NodeProbe.JMX_NOTIFICATION_POLL_INTERVAL, TimeUnit.MILLISECONDS))
            {
                queryForCompletedRepair(String.format("After waiting for poll interval of %s milliseconds",
                                                      NodeProbe.JMX_NOTIFICATION_POLL_INTERVAL));
            }
            if (error != null)
            {
                throw error;
