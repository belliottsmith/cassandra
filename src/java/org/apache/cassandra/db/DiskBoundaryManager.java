    ...
    private static DiskBoundaries getDiskBoundaryValue(ColumnFamilyStore cfs)
    {
        ...
        do
        {
                ...
                && !StorageService.isReplacingSameAddress()) // When replacing same address, the node marks itself as UN locally
            {
                ...
                localRanges = tmd.getPendingRanges(cfs.keyspace.getName(), FBUtilities.getBroadcastAddressAndPort()).toRangeSet();
            }
            else
            {
                ...
                localRanges = cfs.keyspace.getReplicationStrategy().getAddressReplicas(tmd.cloneAfterAllSettled(), FBUtilities.getBroadcastAddressAndPort()).toRangeSet();
            }
