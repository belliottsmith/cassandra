...
public abstract class ReadCommand implements ReadQuery
{
    ...
    public static final String TRACK_WARNINGS = "TrackWarnings";
    public static final String TOMBSTONE_WARNING = "TSW";
    public static final String TOMBSTONE_ABORT = "TSA";

    public static final String INDEX_SIZE_WARNING = "ISW";
    public static final String INDEX_SIZE_ABORT = "ISA";
    ...
    public UnfilteredPartitionIterator executeLocally(ReadOrderGroup orderGroup)
    {
        ...
        try
        {
            if (!DatabaseDescriptor.isTombstoneCountGCable())
    ...
    private UnfilteredPartitionIterator withMetricsRecording(UnfilteredPartitionIterator iter, final TableMetrics metric, final long startTimeNanos)
    {
        class MetricRecording extends Transformation<UnfilteredRowIterator>
        {
            ...
            public Row applyToRow(Row row)
            {
                ...
                // we might have already counted a cell tombstone here, that would mean it was
                // written after the row tombstone was written and we should count both.
            ...
            private void countTombstone(ClusteringPrefix clustering)
            {
                ...
                if (tombstones > failureThreshold && respectTombstoneThresholds)
                {
                    ...
                    MessageParams.remove(TOMBSTONE_WARNING);
                    MessageParams.add(TOMBSTONE_ABORT, tombstones);
            ...
            public void onClose()
            {
                ...
                if (warnTombstones)
                {
                    ...
                    if (trackWarnings)
                        MessageParams.add(TOMBSTONE_WARNING, tombstones);
                    else
    ...
    InputCollector<UnfilteredRowIterator> iteratorsForPartition(ColumnFamilyStore.ViewFragment view)
    {
        final BiFunction<List<UnfilteredRowIterator>, RepairedDataInfo, UnfilteredRowIterator> merge =
            (unfilteredRowIterators, repairedDataInfo) -> {
                UnfilteredRowIterator repaired = UnfilteredRowIterators.merge(unfilteredRowIterators, nowInSec());
                return repairedDataInfo.withRepairedDataInfo(repaired);
            };

        // For the purposes of overreading repaired data to provide a digest, single partition reads may consume
        // additional rows from the partition. The merged row iterator of repaired rows will be consumed until it
        // is exhausted or the RepairedDataInfo's internal counter is satisfied. No additional partitions are
        // required (as this is a single partition read), so here we supply an null iterator which RepairedDataInfo
        // handles safely.
        final Function<UnfilteredRowIterator, UnfilteredPartitionIterator> postLimitPartitions = (rows) -> null;
    ...
    static class InputCollector<T extends AutoCloseable>
    {
                       ...
                       boolean isTrackingRepairedStatus)
        {
            ...
            if (repairedSSTables == null)
            {
                ...
                unrepairedIters = new ArrayList<>(view.sstables.size() + Iterables.size(view.memtables) + 1);
            }
            else
            {
