...
public class CompactionController implements AutoCloseable
{
    ...
    {
        return getFullyExpiredSSTables(cfs, compacting, overlappingSSTables, gcBefore, repairTimeSnapshot, ignoreOverlaps());
    }
        ...
        if (ignoreOverlaps)
        {
            ...
            for (SSTableReader candidate : compacting)
            {
                ...
                if (candidate.getSSTableMetadata().maxLocalDeletionTime < Math.min(lastRepairTime, gcBefore))
                {
                    ...
                    logger.info("Dropping overlap ignored expired SSTable {} (maxLocalDeletionTime={}, gcBefore={})",
    ...
    public static Set<SSTableReader> getFullyExpiredSSTables(ColumnFamilyStore cfStore,
                                                             ...
                                                             SuccessfulRepairTimeHolder repairTimeHolder)
    {
        return getFullyExpiredSSTables(cfStore, compacting, overlapping, gcBefore, repairTimeHolder, false);
    }
