...
public class CompactionIterator extends CompactionInfo.Holder implements UnfilteredPartitionIterator
{
                              ...
                              TopPartitionTracker.Collector topPartitionCollector)
    {
        ...
        sstables = ImmutableSet.copyOf(scanners.stream().map(ISSTableScanner::getBackingSSTables).flatMap(Collection::stream).collect(Collectors.toSet()));
    ...
    private class Purger extends PurgeFunction
    {
        ...
        private Purger(boolean isForThrift, CompactionController controller, int nowInSec)
        {
            ...
            super(controller.cfs,
                  ...
                  controller.getRepairTimeSnapshot());
            this.controller = controller;
        }
