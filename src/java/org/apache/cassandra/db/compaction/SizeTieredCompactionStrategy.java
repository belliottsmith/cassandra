...
public class SizeTieredCompactionStrategy extends AbstractCompactionStrategy
{
    ...
    public synchronized AbstractCompactionTask getNextBackgroundTask(int gcBefore)
    {
        ...
        while (true)
        {
            ...
            Transaction transaction = cfs.getDataTracker().tryModify(hottestBucket, OperationType.COMPACTION);
            if (transaction != null)
    ...
    public Collection<AbstractCompactionTask> getMaximalTask(final int gcBefore, boolean splitOutput)
    {
        ...
        if (Iterables.isEmpty(filteredSSTables))
            return null;
        Transaction txn = cfs.getDataTracker().tryModify(filteredSSTables, OperationType.COMPACTION);
        if (txn == null)
            return null;
        if (splitOutput)
    ...
    public AbstractCompactionTask getUserDefinedTask(Collection<SSTableReader> sstables, final int gcBefore)
    {
        ...
        Transaction transaction = cfs.getDataTracker().tryModify(sstables, OperationType.COMPACTION);
        if (transaction == null)
        {
    ...
    {
        public SplittingCompactionTask(ColumnFamilyStore cfs, Transaction txn, int gcBefore, boolean offline)
        {
            super(cfs, txn, gcBefore, offline);
        }

        @Override
        public CompactionAwareWriter getCompactionAwareWriter(ColumnFamilyStore cfs, Transaction txn, Set<SSTableReader> nonExpiredSSTables)
        {
