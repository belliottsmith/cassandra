...
public class UnfilteredValidation
{
    ...
    public static void maybeValidateUnfiltered(Unfiltered unfiltered, CFMetaData metadata, DecoratedKey key, SSTableReader sstable)
    {
    ...
    public static void handleInvalid(CFMetaData metadata, DecoratedKey key, SSTableReader sstable, String invalidContent)
    {
        Config.CorruptedTombstoneStrategy strat = DatabaseDescriptor.getCorruptedTombstoneStrategy();
        String keyString;
        try
        {
            keyString = metadata.getKeyValidator().getString(key.getKey());
        }
        catch (Throwable t)
        {
        ...
        switch (strat)
        {
            ...
            default:
                throw new AssertionError("Unrecognized strategy " + strat);
        }
    }
}
