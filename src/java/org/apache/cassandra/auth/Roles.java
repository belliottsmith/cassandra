...
public class Roles
{
    private static final Role NO_ROLE = new Role("",
                                                 false,
                                                 false,
     ...
     */
    public static Set<Role> getGrantedRoles(RoleResource primaryRole)
    {
    ...
    {
        for (Role r : getGrantedRoles(role))
            if (r.isSuper)
                return true;
    ...
    {
        return Iterables.tryFind(getGrantedRoles(role), r -> r.resource.equals(role)).or(NO_ROLE).canLogin;
    }
    ...
    {
        return Iterables.tryFind(getGrantedRoles(role), r -> r.resource.equals(role)).or(NO_ROLE).options;
    }
     ...
     * This is used by the default implementation of IRoleManager#getGrantedRoles so that IRoleManager impls
     * which don't implement an optimized getGrantedRoles remain compatible.
