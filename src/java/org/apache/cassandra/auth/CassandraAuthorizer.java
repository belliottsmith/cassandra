...
public class CassandraAuthorizer implements IAuthorizer
{
            ...
            throws RequestExecutionException
    {
        process(String.format("UPDATE %s.%s SET permissions = permissions %s {%s} WHERE role = '%s' AND resource = '%s'",
                ...
                AuthProperties.instance.getWriteConsistencyLevel());
    }
    ...
    {
        Set<PermissionDetails> details = new HashSet<>();
        for (UntypedResultSet.Row row : process(buildListQuery(resource, role), AuthProperties.instance.getReadConsistencyLevel()))
        {
    ...
    {
        QueryOptions options = QueryOptions.forInternalCalls(AuthProperties.instance.getWriteConsistencyLevel(), Collections.emptyList());
