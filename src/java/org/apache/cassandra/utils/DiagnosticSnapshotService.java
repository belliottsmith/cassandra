...
public class DiagnosticSnapshotService
{
    ...
    public static void repairedDataMismatch(CFMetaData metadata, Iterable<InetAddress> replicas)
    {
    ...
    private static class DiagnosticSnapshotTask implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                ...
                if (ks == null)
                {
                    logger.info("Diagnostic snapshot request received from {} for {}.{} but keyspace not found",
                                from,
                                command.keyspace,
                ...
                {
                    logger.info("Diagnostic snapshot request received from {} for {}.{}, " +
                ...
                logger.info("Diagnostic snapshot request received from {} for {}.{} tag: {}",
                            from,
                            command.keyspace,
                            command.column_family,
                            command.snapshot_name);
            ...
            {
                logger.warn("Diagnostic snapshot request received from {} for {}.{} but CFS not found",
                            from,
                            command.keyspace,
                            command.column_family);
            }
