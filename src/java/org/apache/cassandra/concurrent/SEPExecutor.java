...
public class SEPExecutor extends AbstractLocalAwareExecutorService implements SEPExecutorMBean
{
    ...
    //  the number of work permits will only ever be negative during a resize operation.  During
    //  normal operation they should be non-negative.
    ...
    public synchronized void shutdown()
    {
        ...
        // release metrics
        metrics.release();
        MBeanWrapper.instance.unregisterMBean(mbeanName, MBeanWrapper.OnException.LOG);
    }
    ...
    public synchronized void setMaximumPoolSize(int newMaximumPoolSize)
    {
        ...
        // If resizing a busy pool that currently has more work permits taken than the new size allows,
        // the number of available work permits can briefly go negative until enough SEPWorkers
        // attempt to take new task permits and discover the executor pool has too many workers and exit,
        // returning their work permits.
        ...
        {
            logger.error(String.format("Calling the %s shared executor pool update maximum pool size listener from %d to %d threw an exception",
                                       name, oldMaximumPoolSize, newMaximumPoolSize), tr);
        }
    }
