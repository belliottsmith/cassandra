...
import static org.apache.cassandra.concurrent.SEPExecutor.TakeTaskPermitResult.SHRINKING;
    ...
    public void run()
    {
        ...
        try
        {
            while (true)
            {
                ...
                set(Work.WORKING);
                boolean shutdown;
                SEPExecutor.TakeTaskPermitResult permitStatus = TOOK_PERMIT; // make sure set if shutdown check short circuits
                while (true)
                {
                    ...
                    if ((shutdown = assigned.shuttingDown) || (permitStatus=assigned.takeTaskPermit(true)) != TOOK_PERMIT)
                        break;
                    task = assigned.tasks.poll();
                }

                // return our work permit, and maybe signal shutdown
                if (permitStatus != SHRINKING)
