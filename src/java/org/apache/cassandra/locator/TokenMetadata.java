                                                           ...
                                                           Set<Pair<Token, InetAddressAndPort>> movingEndpoints)
    {
        ...
        for (Range<Token> range : removeAffectedRanges)
        {
            ...
            for (Replica replica : newReplicas)
            {
                ...
                // TODO: TR-Review why don't transient ranges need a pending state? if a transient ownership range is moving,
                // for correctness the new owner of the range needs to replicate the old transient range before can be safely considered for participating in reads?
