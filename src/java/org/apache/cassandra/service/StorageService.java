...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    private synchronized UUID prepareReplacementInfo() throws ConfigurationException
    {
        ...
        try
        {
            ...
            if (state.isEmptyWithoutStatus() && Boolean.parseBoolean(System.getProperty("cassandra.allow_empty_replace_address", "true")))
            {
    ...
    private static Collection<Token> validateReplacementBootstrapTokens(TokenMetadata tokenMetadata,
                                                                        InetAddress replaceAddress,
                                                                        Collection<Token> bootstrapTokens)
    {
        Map<Token, InetAddress> conflicts = new HashMap<>();
        for (Token token : bootstrapTokens)
        {
            InetAddress conflict = tokenMetadata.getEndpoint(token);
    ...
    public void setManualSeverity(double incr)
    {
        bgMonitor.setManualSeverity(incr);
    }
    ...
    {
        return bgMonitor.getManualSeverity();
    }
    ...
    */
    ...
    public boolean isReplacingSameHostAddressAndHostId(UUID hostId)
    {
        ...
        {
            // ACI Cassandra: If a host is decomissioned and the DNS entry is removed before the
            ...
            //
            // The original host replacement must have been able to resove the replacing address on startup
            ...
            // See rdar://74366262 (Nodes stuck in JOINING after replacement)
    ...
    public int upgradeSSTables(String keyspaceName, boolean excludeCurrentVersion, long maxSSTableTimestamp, int jobs, String... tableNames) throws IOException, ExecutionException, InterruptedException
    {
        ...
        {
            CompactionManager.AllSSTableOpStatus oneStatus = cfStore.sstablesRewrite(excludeCurrentVersion, maxSSTableTimestamp, jobs);
            if (oneStatus != CompactionManager.AllSSTableOpStatus.SUCCESSFUL)
                status = oneStatus;
        }
        return status.statusCode;
    ...
    */
    ...
    {
        // will get picked up next time the BackgroundActivityReporter runs
        DatabaseDescriptor.setDynamicManualSeverityOnly(dynamicManualSeverityOnly);
    }

    public boolean getDynamicManualSeverityOnly()
    {
        return DatabaseDescriptor.getDynamicManualSeverityOnly();
    }
    ...
    {
        return DatabaseDescriptor.getDynamicBadnessThreshold();
    }
    ...
    public void enableReplicaFilteringProtection()
    {
        ...
        logger.info("replica filtering protection enabled");
    }
    
    public void disableReplicaFilteringProtection()
    {
        DatabaseDescriptor.setReplicaFilteringProtectionEnabled(false);
        logger.info("replica filtering protection disabled");
    }
    
    public boolean isReplicaFilteringProtectionEnabled()
    {
        return DatabaseDescriptor.isReplicaFilteringProtectionEnabled();
    }
    ...
    {
        if (ActiveRepairService.instance.getSessions(false).size() > 0)
        {
