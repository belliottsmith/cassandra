...
public class DebugBlockedTxns
{
    ...
    private void visit(long deadlineNanos) throws TimeoutException
    {
        ...
        Promise<List<Txn>> next = AccordService.asPromise(AsyncChains.allOf(chains));
        while (next != null)
        {
            ...
            Promise<List<Void>> awaitKeys = drainToPromise(queuedKeys, (List<AsyncChain<Void>>)(List)tmp);
            if (awaitKeys != null && !awaitKeys.awaitUntilThrowUncheckedOnInterrupt(deadlineNanos))
                throw new TimeoutException();

            next = drainToPromise(queuedTxn, (List<AsyncChain<Txn>>)(List)tmp);
        }
    }

    private <T> Promise<List<T>> drainToPromise(Queue<AsyncChain<T>> drain, List<AsyncChain<T>> tmp)
    {
        AsyncChain<T> next;
        ...
        if (tmp.isEmpty())
            return null;
        Promise<List<T>> result = AccordService.asPromise(AsyncChains.allOf(List.copyOf(tmp)));
        tmp.clear();
        return result;
    }
    ...
    {
        return commandStore.build(PreLoadContext.contextFor(txnId, "Populate txn_blocked_by"), safeStore -> {
    ...
    {
        return commandStore.build(PreLoadContext.contextFor(RoutingKeys.of(key.toUnseekable()), SYNC, READ_WRITE, "Populate txn_blocked_by"), safeStore -> {
