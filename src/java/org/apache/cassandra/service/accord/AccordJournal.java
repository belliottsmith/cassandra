...
public class AccordJournal implements IJournal, Shutdownable
{
    ...
    private static final ThreadLocal<byte[]> keyCRCBytes = ThreadLocal.withInitial(() -> new byte[23]);
    ...
    public void replay()
    {
        ...
        Map<Key, List<SavedCommand.LoadedDiff>> loaded = new HashMap<>();
        journal.replayStaticSegments(new RecordConsumer<Key>()
        {
            public void accept(long segment, int position, Key key, ByteBuffer buffer, IntHashSet hosts, int userVersion)
            {
        ...
        for (List<SavedCommand.LoadedDiff> diff : loaded.values())
        {
    ...
    private List<SavedCommand.LoadedDiff> loadDiffs(int commandStoreId, TxnId txnId)
    {
        return (List<SavedCommand.LoadedDiff>)(List<?>) journal.readAll(new Key(txnId, Type.SAVED_COMMAND, commandStoreId));
    }

    @Override
    public boolean appendCommand(int commandStoreId, List<SavedCommand.SavedDiff> outcomes, List<Command> sanityCheck, Runnable runnable)
    {
        if (outcomes.isEmpty())
            return false;
        ...
        for (int i = 0; i < outcomes.size(); i++)
        {
            ...
            Key key = new Key(outcome.txnId, Type.SAVED_COMMAND, commandStoreId);
        ...
        {
            journal.onFlush(pointer, runnable);
            return true;
        }
    }
    ...
    public static class Key
    {
        ...
        final Type type; // TODO (desired): do we even need type here anymore?
