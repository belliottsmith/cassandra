...
public class StorageProxy implements StorageProxyMBean
{
    ...
    static
    {
        ...
        counterWriteOnCoordinatorPerformer = (mutation, targets, responseHandler, localDataCenter) ->
        {
            EndpointsForToken selected = targets.contacts().withoutSelf();
            Replicas.temporaryAssertFull(selected); // TODO CASSANDRA-14548
            Stage.COUNTER_MUTATION.getExecutor()
    ...
    throws WriteTimeoutException
    {
        ...
        for (Replica replica: replicaPlan.contacts())
        {
            if (replica.isSelf())
            {
                PAXOS_PREPARE_REQ.stage.getExecutor().execute(() -> {
                    try
                    {
    ...
    throws WriteTimeoutException
    {
        ...
        for (Replica replica : replicaPlan.contacts())
        {
            if (replica.isSelf())
            {
                PAXOS_PROPOSE_REQ.stage.getExecutor().execute(() -> {
                    try
                    {
    ...
    {
        PAXOS_COMMIT_REQ.stage.getExecutor().maybeExecuteImmediately(new LocalMutationRunnable(localReplica)
        {
    ...
    {
        stage.getExecutor().maybeExecuteImmediately(new LocalMutationRunnable(localReplica)
        {
            public void runMayThrow()
            {
                try
    ...
    private static class RangeCommandIterator extends AbstractIterator<RowIterator> implements PartitionIterator
    {
        ...
        private SingleRangeResponse query(ReplicaPlan.ForRangeRead replicaPlan, boolean isFirst)
        {
            ...
            {
                Stage.READ.getExecutor().execute(new LocalReadRunnable(rangeCommand, handler));
            }
            else
            {
                for (Replica replica : replicaPlan.contacts())
    ...
    private static Future<Void> submitHint(HintRunnable runnable)
    {
        ...
        return (Future<Void>) Stage.MUTATION.getExecutor().submit(runnable);
    }
