...
public class CIEFileSystemOwnershipCheck extends FileSystemOwnershipCheck
{
    ...
    protected String constructTokenFromProperties(Map<String, Object> config) throws StartupException
    {
        ...
        if (null == app || app.isEmpty())
            throw exception(String.format(MISSING_PROPERTY, APP_NAME_PROPERTY));

        String cluster = System.getProperty(CLUSTER_NAME_PROPERTY);
        if (null == cluster || cluster.isEmpty())
            throw exception(String.format(MISSING_PROPERTY, CLUSTER_NAME_PROPERTY));

        String instance = System.getProperty(INSTANCE_ID_PROPERTY);
        if (null == instance || instance.isEmpty())
            throw exception(String.format(MISSING_PROPERTY, INSTANCE_ID_PROPERTY));
