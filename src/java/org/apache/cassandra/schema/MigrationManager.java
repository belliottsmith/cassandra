...
public class MigrationManager
{
    ...
    private static Future<?> submitMigrationTask(InetAddressAndPort endpoint)
    {
         ...
         */
        return MIGRATION.getExecutor().submit(new MigrationTask(endpoint));
    }
    ...
    {
        Future<?> f = MIGRATION.getExecutor().submit(() -> Schema.instance.mergeAndAnnounceVersion(schema));
    ...
    public static KeyspacesDiff announce(SchemaTransformation transformation, boolean locally)
    {
        ...
        Future<Schema.TransformationResult> future =
            MIGRATION.getExecutor().submit(() -> Schema.instance.transform(transformation, locally, now));
