...
public class ReduceHelper
{
    ...
    public static ImmutableMap<InetAddress, HostDifferences> reduce(DifferenceHolder differences, PreferedNodeFilter filter)
    {
        ...
        for (Map.Entry<InetAddress, IncomingRepairStreamTracker> trackerEntry : trackers.entrySet())
        {
            ...
            for (Map.Entry<Range<Token>, StreamFromOptions> entry : tracker.getIncoming().entrySet())
            {
                ...
                for (InetAddress remoteNode : pickConsistent(trackerEntry.getKey(), entry.getValue(), filter))
    ...
    private static final Comparator<InetAddress> comparator = Comparator.comparing(InetAddress::getHostAddress);
    /**
     ...
     */
    private static Collection<InetAddress> pickConsistent(InetAddress streamingNode,
                                                          ...
                                                          PreferedNodeFilter filter)
    {
        ...
        {
            List<InetAddress> toSearch = new ArrayList<>(filter.apply(streamingNode, toStream));
            if (toSearch.isEmpty())
