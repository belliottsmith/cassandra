...
public class PendingAntiCompaction
{
        ...
        public boolean apply(SSTableReader sstable)
        {
            ...
            if (cis != null && !cis.isEmpty())
            {
                ...
                for (CompactionInfo ci : cis)
                    sb.append(ci.compactionId() == null ? "no compaction id" : ci.compactionId()).append(':').append(ci.getSSTables()).append(',');
    ...
    public static class AcquisitionCallable implements Callable<AcquireResult>
    {
        ...
        private AcquireResult acquireTuple()
        {
            ...
            try
            {
                ...
                Set<SSTableReader> sstables = Sets.newHashSet(Iterables.filter(cfs.getLiveSSTables(), predicate));
                if (sstables.isEmpty())
    ...
    static class AcquisitionCallback implements AsyncFunction<List<AcquireResult>, Object>
    {
        ...
        private final Collection<Range<Token>> tokenRanges;
        private final BooleanSupplier isCancelled;

        public AcquisitionCallback(UUID parentRepairSession, Collection<Range<Token>> tokenRanges, BooleanSupplier isCancelled)
        {
        ...
        {
            return CompactionManager.instance.submitPendingAntiCompaction(result.cfs, tokenRanges, result.refs, result.txn, parentRepairSession, parentRepairSession, isCancelled);
        }
    ...
    {
        this(prsId, ranges, ACQUIRE_RETRY_SECONDS, ACQUIRE_SLEEP_MS, executor, isCancelled);
    }
    ...
    public ListenableFuture run() throws NoSuchRepairSessionException
    {
        ...
        for (ColumnFamilyStore cfs : prs.getColumnFamilyStores())
        {
            ...
            ListenableFutureTask<AcquireResult> task = ListenableFutureTask.create(getAcquisitionCallable(cfs, ranges, prsId, acquireRetrySeconds, acquireSleepMillis));
    ...
    @VisibleForTesting
    protected AcquisitionCallable getAcquisitionCallable(ColumnFamilyStore cfs, Collection<Range<Token>> ranges, UUID prsId, int acquireRetrySeconds, int acquireSleepMillis)
    {
