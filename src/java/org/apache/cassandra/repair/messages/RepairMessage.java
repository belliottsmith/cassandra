...
{
    private static final CassandraVersion SUPPORTS_TIMEOUTS = new CassandraVersion("4.0.0.52"); // todo: update to be correct
        ...
        RequestCallback<?> callback = new RequestCallback<Object>()
        {
            ...
            public void onFailure(InetAddressAndPort from, RequestFailureReason failureReason)
            {
                ...
                if (supportsTimeouts(from, request.desc.parentSessionId))
                    failureCallback.onFailure(RepairException.warn(String.format("Got %s failure from %s: %s", verb, from, failureReason)));
            }
        };

        MessagingService.instance().sendWithCallback(Message.out(verb, request)
                                                            .withFlag(CALL_BACK_ON_FAILURE),
                                                     endpoint,
                                                     callback);
    }
