 ...
 * by the specs.
 *
 * Backport: Apple Internal from trunk. This class was not here in 3.0.
 */
public enum ProtocolVersion implements Comparable<ProtocolVersion>
{
    ...
    private final static ProtocolVersion[] SUPPORTED_VERSIONS = new ProtocolVersion[] { V3, V4 };
        ...
        if (ret == null)
        {
            ...
            for (ProtocolVersion version : UNSUPPORTED)
            {
                ...
                if (version.num == versionNum)
                    throw new ProtocolException(ProtocolVersion.invalidVersionMessage(versionNum), version.num);
            }

            // If the version is invalid reply with the highest version that we support
            throw new ProtocolException(invalidVersionMessage(versionNum), MAX_SUPPORTED_VERSION.num);
        }
