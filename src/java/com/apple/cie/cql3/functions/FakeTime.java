...
public abstract class FakeTime
{
    ...
    public static TimeUUID fakeNow()
    {
        synchronized (FakeTime.class)
        {
            assert (fakeCentiNanos < 10_000_000);
            long unixEpochMicros = fakeInstant.getEpochSecond() * 1_000_000 + fakeInstant.getNano() / 1000L;
            return TimeUUID.atUnixMicrosWithLsb(unixEpochMicros, fakeCentiNanos++);
        }
    }
