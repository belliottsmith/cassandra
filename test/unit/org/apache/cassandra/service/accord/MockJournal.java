...
public class MockJournal implements IJournal
{
    ...
    @Override
    public boolean appendCommand(int commandStoreId, List<SavedCommand.SavedDiff> diffs, List<Command> sanityCheck, Runnable onFlush)
    {
        Type type = Type.SAVED_COMMAND;
        for (SavedCommand.SavedDiff diff : diffs)
        {
            Key key = new Key(diff.txnId, type, commandStoreId);
