...
public class SimulatedAccordOperationTest extends SimulatedAccordCommandStoreTestBase
{
    ...
    private static class SimulatedLoadFunctionWrapper implements FunctionWrapper
    {
        ...
        @Override
        public <I, O> Function<I, O> wrap(Function<I, O> f)
        {
            return new SimulatedLoadFunction<>(f, actions, delayNanos);
        }
    }

    private static class SimulatedLoadFunction<K, V> implements Function<K, V>
    {
        private final Function<K, V> load;
        private final Supplier<Action> actions;
        private final LongSupplier delaysNanos;
        SimulatedLoadFunction(Function<K, V> load, Supplier<Action> actions, LongSupplier delaysNanos)
        {
            this.load = load;
            this.actions = actions;
        ...
        public V apply(K key)
        {
            ...
            if (action == Action.SUCCESS) return load.apply(key);
            throw new SimulatedFault("Failure loading " + key);
        }
    }
}
