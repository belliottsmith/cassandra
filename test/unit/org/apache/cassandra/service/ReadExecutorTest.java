...
public class ReadExecutorTest
{
    ...
    public void testUnableToSpeculate() throws Throwable
    {
        ...
        AbstractReadExecutor executor = new AbstractReadExecutor.NeverSpeculatingReadExecutor(ks, cfs, new MockSinglePartitionReadCommand(), ConsistencyLevel.LOCAL_QUORUM, targets, true);
        ...
        //Shouldn't increment
        executor = new AbstractReadExecutor.NeverSpeculatingReadExecutor(ks, cfs, new MockSinglePartitionReadCommand(), ConsistencyLevel.LOCAL_QUORUM, targets, false);
        executor.maybeTryAdditionalReplicas();
        try
        {
            executor.get();
    ...
    public void testSpeculateSucceeded() throws Throwable
    {
        ...
        AbstractReadExecutor executor = new AbstractReadExecutor.SpeculatingReadExecutor(ks, cfs, new MockSinglePartitionReadCommand(TimeUnit.DAYS.toMillis(365)), ConsistencyLevel.LOCAL_QUORUM, targets);
        executor.maybeTryAdditionalReplicas();
        new Thread()
        {
            @Override
    ...
    public void testSpeculateFailed() throws Throwable
    {
        ...
        assertEquals(0, ks.metric.speculativeRetries.getCount());
        assertEquals(0, ks.metric.speculativeFailedRetries.getCount());
        AbstractReadExecutor executor = new AbstractReadExecutor.SpeculatingReadExecutor(ks, cfs, new MockSinglePartitionReadCommand(), ConsistencyLevel.LOCAL_QUORUM, targets);
        executor.maybeTryAdditionalReplicas();
        try
        {
            executor.get();
    ...
    public static class MockSinglePartitionReadCommand extends SinglePartitionReadCommand
    {
        ...
        {
            super(false, 0, false, cfs.metadata, 0, null, null, null, Util.dk("ry@n_luvs_teh_y@nk33z"), null);
