...
public class PaxosBallotTrackerTest
{
    ...
    private static void testHighBound(Stage stage, Order order)
    {
        ...
        UUID ballot1 = Paxos.ballotForConsistency(1001, ConsistencyLevel.SERIAL);
        UUID ballot2 = Paxos.ballotForConsistency(1002, ConsistencyLevel.SERIAL);
    ...
    public void lowBoundAccept() throws IOException
    {
        ...
        try (PaxosState state = PaxosState.get(key, cfm, ballot2))
        {
            UUID result = state.acceptIfLatest(new Commit.Proposal(ballot2, PartitionUpdate.emptyUpdate(cfm, key)));
