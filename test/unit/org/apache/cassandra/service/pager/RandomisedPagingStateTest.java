...
public class RandomisedPagingStateTest
{
    ...
    public void testFormatChecksPkAndCk()
    {
        ...
        CFMetaData metadata = CFMetaData.Builder.create("ks", "tbl")
                                                ...
                                                .addRegularColumn("v", LongType.instance)
                                                .build();
        ColumnDefinition def = metadata.getColumnDefinition(new ColumnIdentifier("ck", false));
        ...
        for (int i = 0; i < ROUNDS; i++)
        {
            ...
            Clustering c = new Clustering(ByteBufferUtil.bytes("ck"), ckBytes);
    ...
    {
        PagingState.RowMark mark = PagingState.RowMark.create(metadata, row, ProtocolVersion.V3.asInt(), true);
        ByteBuffer pkBytes = ByteBuffer.allocate(rnd.nextInt(maxPkSize) + 1);
        for (int j = 0; j < pkBytes.limit(); j++)
            pkBytes.put((byte) rnd.nextInt());
        pkBytes.flip().rewind();

        PagingState state = new PagingState(pkBytes, mark, rnd.nextInt(MAX_REMAINING) + 1, rnd.nextInt(MAX_REMAINING) + 1, true);
