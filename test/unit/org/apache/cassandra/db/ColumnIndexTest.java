...
public class ColumnIndexTest extends CQLTester
{
    ...
    public void testListDownsampling()
    {
        ...
        for(int i = 0; i < 101; i++)
        {
            columnIndex.add(new IndexHelper.IndexInfo(cn(5L * i), cn(5L * i + 5), lastIndexEnd, 10, deletionInfo, cfMeta.comparator));
            lastIndexEnd += 10;
        }
        ...
        long expectedOffset = columnIndex.get(100).getOffset();
        long expectedWidth = columnIndex.get(100).getWidth();
        ...
        for(int i = 0; i < 50; i++)
        {
            Assert.assertEquals(20 * i, columnIndex.get(i).getOffset());
            Assert.assertEquals(20, columnIndex.get(i).getWidth());
        }

        // last index shouldn't merge, so it should be the same as the original index
        Assert.assertEquals(expectedOffset, columnIndex.get(columnIndex.size() - 1).getOffset());
        Assert.assertEquals(expectedWidth, columnIndex.get(columnIndex.size() - 1).getWidth());
        ...
        expectedOffset = columnIndex.get(columnIndex.size() - 3).getOffset();
        expectedWidth = columnIndex.subList(columnIndex.size() - 3, columnIndex.size())
                                   .stream()
                                   .mapToLong(IndexHelper.IndexInfo::getWidth)
                                   .sum();
        ...
        for(int i = 1; i < 12; i++)
        {
            ...
            Assert.assertEquals(((20 * 4) * i), columnIndex.get(i).getOffset());
            Assert.assertEquals(20 * 4, columnIndex.get(i).getWidth());
        }
