...
public class ScrubTest
{
    ...
    public void testScrubCorruptedCounterRow() throws IOException, WriteTimeoutException
    {
        ...
        try (Transaction txn = cfs.getDataTracker().tryModify(Arrays.asList(sstable), OperationType.SCRUB);
             Scrubber scrubber = new Scrubber(cfs, txn, false, false);)
        {
            scrubber.scrub();
        ...
        try (Transaction txn = cfs.getDataTracker().tryModify(Arrays.asList(sstable), OperationType.SCRUB);
             Scrubber scrubber = new Scrubber(cfs, txn, true, false);)
        {
    ...
    public void testScrubOutOfOrder() throws Exception
    {
        ...
        */
        ...
        try (Transaction txn = Transaction.offline(OperationType.SCRUB, sstable);
