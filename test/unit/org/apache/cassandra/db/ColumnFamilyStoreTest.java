...
//    }
    ...
    public void testSerializationDifferences() throws Exception
    {
        ...
        String query = String.format("SELECT * FROM %s.%s WHERE keyspace_name='%s' AND columnfamily_name='%s'",
                                     SystemKeyspace.NAME, SystemKeyspace.REPAIR_HISTORY_CF, KEYSPACE1, CF_STANDARD1);
        ...
        String insert = String.format("INSERT INTO %s.%s (keyspace_name, columnfamily_name, range, succeed_at) VALUES ('%s', '%s', 0x%s, %s)",
                                      SystemKeyspace.NAME, SystemKeyspace.REPAIR_HISTORY_CF,
    ...
    public void testSnapshotWithoutFlushWithSecondaryIndexes() throws Exception
    {
        ...
        // CIE - we include the ks-table in the name, and for 2i CFS this means the pattern is
        // ks-table.index; which will not match the ks-table pattern of the base table
        String[] baseParts = baseTableFile.split("-", 3);
        assert indexTableFile.contains(baseParts[0]); // ks;
        assert indexTableFile.contains(baseParts[1]); // table;
        assert indexTableFile.contains(baseParts[2]); // rest;
    }
}
