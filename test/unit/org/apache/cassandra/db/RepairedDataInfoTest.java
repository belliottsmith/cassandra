...
public class RepairedDataInfoTest
{
    ...
    static final ColumnDefinition valueMetadata;
    static final ColumnDefinition staticMetadata;

    static
    {
        String ks = "repaired_data_info_test";
        cfs = MockSchema.newCFS(ks, RepairedDataInfoTest::addStaticColumnDef);
        metadata = cfs.metadata;
        valueMetadata = metadata.partitionColumns().columns(false).getSimple(0);
        staticMetadata = metadata.partitionColumns().columns(true).getSimple(0);
    }

    private static void addStaticColumnDef(CFMetaData cfm)
    {
        cfm.addColumnDefinition(ColumnDefinition.staticDef(cfm, bytes("s"), UTF8Type.instance));
    }
    ...
    public static Cell cell(ColumnDefinition def, Object value)
    {
    ...
    {
        return new RangeTombstoneBoundMarker(Slice.Bound.create(metadata.comparator, true, true, Integer.toString(start)),
                                             new DeletionTime(FBUtilities.timestampMicros(), FBUtilities.nowInSeconds()));
    }

    private RangeTombstoneBoundMarker close(int close)
    {
        return new RangeTombstoneBoundMarker(Slice.Bound.create(metadata.comparator, false, true, Integer.toString(close)),
                                             new DeletionTime(FBUtilities.timestampMicros(), FBUtilities.nowInSeconds()));
    }
    ...
    private UnfilteredRowIterator partitionWithStaticRow(ByteBuffer pk, Row staticRow, Unfiltered... unfiltereds)
    {
        ...
        return new AbstractUnfilteredRowIterator(metadata, dk(pk), DeletionTime.LIVE, metadata.partitionColumns(), staticRow, false, EncodingStats.NO_STATS) {
