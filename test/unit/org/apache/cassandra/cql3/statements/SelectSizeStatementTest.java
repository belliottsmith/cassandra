...
public class SelectSizeStatementTest
{
    ...
    public void commandCreation() throws Exception
    {
        ...
        command = statement.createCommand(QueryOptions.forInternalCalls(ConsistencyLevel.QUORUM, Lists.newArrayList(ByteBufferUtil.bytes(1))), clientState);
        ...
        command = statement.createCommand(QueryOptions.forInternalCalls(ConsistencyLevel.QUORUM, Lists.newArrayList()), clientState);
        Assert.assertEquals("ks", command.keyspace);
        Assert.assertEquals("tbl", command.table);
        Assert.assertEquals(ByteBufferUtil.bytes(1), command.key);
    }
