...
public class ToolRunner
{
    ...
    private static final ImmutableList<String> DEFAULT_CLEANERS = ImmutableList.of("(?im)^picked up.*\\R",
                                                                                   ...
                                                                                   "(?im)^.*LEAK DETECTED: a reference .* was not released before the reference was garbage collected\\R", // remove after resolving rdar://96019696
                                                                                   "(?im)^.*Allocate trace .*:\\RThread.*\\R(\\h+at .*(\\n|$))+", // remove after resolving rdar://96019696
                                                                                   "(?im)^.*epoll not available\\Rjava.lang.UnsupportedOperationException: Native transport was explicit disabled with -Dio.netty.transport.noNative=true\\R(\\h+at .*(\\R|$))+", // suspect extra logging being picked up by log4j, includes stacktrace
