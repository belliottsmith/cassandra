...
public class HintWriteTTLTest
{
    ...
    private static Hint makeHint(CFMetaData cfm, int key, int creationTime, int gcgs)
    {
        PartitionUpdate update = PartitionUpdate.fullPartitionDelete(cfm,
    ...
    public static void setupClass() throws Exception
    {
        ...
        CFMetaData cfm = CFMetaData.compile("CREATE TABLE tbl (k INT PRIMARY KEY, v INT)", "ks").gcGraceSeconds(GC_GRACE);
        SchemaLoader.createKeyspace("ks", KeyspaceParams.simple(1), cfm);

        int nowInSeconds = FBUtilities.nowInSeconds();
        liveHint = makeHint(cfm, 1, nowInSeconds, GC_GRACE);
        ttldHint = makeHint(cfm, 2, nowInSeconds - (TTL + 1), GC_GRACE);
    ...
    {
        Assert.assertTrue(Hint.isLive(s2m(1000), s2m(500), 501, 501)); // still alive
        Assert.assertFalse(Hint.isLive(s2m(1000), s2m(500), 499, 501)); // ttl'd
        Assert.assertFalse(Hint.isLive(s2m(1000), s2m(500), 501, 499)); // gc'd
    }

    @Test
    public void hintIsAlive() throws Exception
    {
