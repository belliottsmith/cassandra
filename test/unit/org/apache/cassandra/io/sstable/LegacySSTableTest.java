    ...
    public void testMutateLevel() throws Exception
    {
        ...
        for (String legacyVersion : legacyVersions)
        {
            ...
            for (ColumnFamilyStore cfs : Keyspace.open("legacy_tables").getColumnFamilyStores())
            {
                for (SSTableReader sstable : cfs.getLiveSSTables())
                {
                    sstable.descriptor.getMetadataSerializer().mutateLevel(sstable.descriptor, LeveledManifest.MAX_LEVEL_COUNT - 1);
                    sstable.reloadSSTableMetadata();
                    ...
                    assertEquals(LeveledManifest.MAX_LEVEL_COUNT - 1, sstable.getSSTableLevel());
                }
            }
        }
    }
