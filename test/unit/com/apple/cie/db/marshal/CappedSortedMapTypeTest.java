...
public class CappedSortedMapTypeTest extends CQLTester
{
    ...
    public void basicFunctionality() throws Throwable
    {
        ...
        final TimeUUID cap = CappedSortedMap.withCap(3);
        for (int msgIdx = 1; msgIdx <= 5; msgIdx++)
        {
            TimeUUID messageId = TimeUUID.fromUuid(UUIDs.startOf(start + msgIdx));
        ...
        assertRows(execute("SELECT pk, ck, csm FROM %s"),
                   row(pk, ck,
                       map(
                       TimeUUID.fromUuid(UUIDs.startOf(start + 3)), 3,
                       TimeUUID.fromUuid(UUIDs.startOf(start + 4)), 4,
                       TimeUUID.fromUuid(UUIDs.startOf(start + 5)), 5)
                   ));
    }

    static private TimeUUID makeMessageId(int msgIdx)
    {
        final long start = 0L;
        return TimeUUID.fromUuid(UUIDs.startOf(start + msgIdx));
    }
    ...
    public void varyCap() throws Throwable
    {
        ...
        int pk = 1;
        int ck = 1;
        final TimeUUID cap1 = CappedSortedMap.withCap(1);
        final TimeUUID cap2 = CappedSortedMap.withCap(2);
        final TimeUUID cap4 = CappedSortedMap.withCap(4);
    ...
    public void checkCapSurvivesCompactionWithGraceZero() throws Throwable
    {
        ...
        int pk = 1;
        int ck = 1;
        final TimeUUID cap = CappedSortedMap.withCap(1);
    ...
    public void rangeTombstoneCleansUp() throws Throwable
    {
        ...
        {
            TimeUUID messageId = makeMessageId(msgIdx);
