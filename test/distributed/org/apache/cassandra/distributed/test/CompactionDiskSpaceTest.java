...
public class CompactionDiskSpaceTest extends TestBaseImpl
{
    ...
    public static class BB
    {
        ...
        public static void install(ClassLoader cl, Integer node)
        {
            ...
            new ByteBuddy().rebase(FileUtils.SafeFileStore.class)
                           .method(named("getUsableSpace"))
                           .intercept(MethodDelegation.to(BB.class))
                           .make()
                           .load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
        ...
        public static long getUsableSpace(@This FileUtils.SafeFileStore thiz)
        {
            try
            {
                if (sstableDir != null && FileUtils.getFileStore(sstableDir.toPath()).equals(thiz))
