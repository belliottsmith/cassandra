...
public class RepairCoordinatorBase extends TestBaseImpl
{
    ...
    public static void setupCluster() throws IOException
    {
        ...
        CLUSTER = init(Cluster.build(2)
                              .withConfig(c -> c.with(Feature.NETWORK)
                                                .with(Feature.GOSSIP)
                                                .set("enable_secondary_index", true))
                              .start());
    }
    ...
    protected String tableName(String prefix) {
        ...
        if (name.length() > 48)
        {
            // table names in 3.0 can't be 48+ so need to truncate
            int allowed = 48 - postfix.length() - 1;
            name = prefix.substring(0, allowed) + "_" + postfix;
        }
        return name;
    }
