        ...
        try (Cluster cluster = Cluster.build(1)
                                      .withConfig(c ->
                                                  c.with(Feature.values())
                                                   .set("tombstone_warn_threshold", 1)
                                      )
                                      .start())
        {
            init(cluster);
            ...
            node.flush(KEYSPACE);
            for (int i = 0; i < 100; i++)
                node.executeInternal(withKeyspace("DELETE FROM %s.tbl where pk=? and ck1=?"), 0, i);
            ...
            try (com.datastax.driver.core.Cluster driver = JavaDriverUtils.create(cluster, ProtocolVersion.V4);
