...
public class OptimiseStreamsRepairTest extends TestBaseImpl
{
    ...
    public static class BBHelper
    {
        ...
        {
            Map<InetAddress, Map<InetAddress, List<Range<Token>>>> fetching = new HashMap<>();
            for (SyncTask task : tasks)
            {
                ...
                Map<InetAddress, List<Range<Token>>> fetch = fetching.computeIfAbsent(task.firstEndpoint, k -> new HashMap<>());
                fetch.computeIfAbsent(task.secondEndpoint, k -> new ArrayList<>()).addAll(task.rangesToSync);
            }
            // 127.0.0.2 is the node out of sync - make sure it does not receive multiple copies of the same range from the other nodes;
            Map<InetAddress, List<Range<Token>>> node2 = fetching.get(InetAddress.getByName("127.0.0.2"));
            ...
            Set<Range<Token>> node2ToNode1 = new HashSet<>(fetching.get(InetAddress.getByName("127.0.0.1")).get(InetAddress.getByName("127.0.0.2")));
            Set<Range<Token>> node2ToNode3 = new HashSet<>(fetching.get(InetAddress.getByName("127.0.0.3")).get(InetAddress.getByName("127.0.0.2")));
