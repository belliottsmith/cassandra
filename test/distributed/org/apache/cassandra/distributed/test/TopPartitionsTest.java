...
public class TopPartitionsTest extends TestBaseImpl
{
    ...
    public void basicPartitionSizeTest() throws Throwable
    {
                                          ...
                                          .start()))
        {
            ...
            cluster.get(1).runOnInstance(() -> assertEquals(5, Keyspace.open(KEYSPACE).getColumnFamilyStore("tbl").getTopSizePartitions().size()));
            cluster.get(2).runOnInstance(() -> assertEquals(10, Keyspace.open(KEYSPACE).getColumnFamilyStore("tbl").getTopSizePartitions().size()));

            cluster.get(1).runOnInstance(() -> DatabaseDescriptor.setMaxTopSizePartitionCount(35));
            cluster.get(1).nodetool("repair", "-vd", KEYSPACE);
            cluster.get(1).runOnInstance(() -> assertEquals(35, Keyspace.open(KEYSPACE).getColumnFamilyStore("tbl").getTopSizePartitions().size()));
            cluster.get(2).runOnInstance(() -> assertEquals(10, Keyspace.open(KEYSPACE).getColumnFamilyStore("tbl").getTopSizePartitions().size()));
        }
    }
    ...
    public void booleanTest() throws Throwable
    {
                                          ...
                                          .start()))
        {
            ...
            cluster.forEach(i -> i.runOnInstance(() -> {
                ...
                SystemKeyspace.getTopPartitions(cfs().metadata, "SIZES");
