...
public class RepairDigestTrackingTest extends TestBaseImpl implements Serializable
{
    ...
    public void testInconsistenciesFound() throws Throwable
    {
        try (Cluster cluster = init(Cluster.create(2)))
        {
            ...
            assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE, ConsistencyLevel.ALL), expected);
            ...
            assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE + " WHERE k=0", ConsistencyLevel.ALL), expected);
    ...
    public void testPurgeableTombstonesAreIgnored() throws Throwable
    {
        try (Cluster cluster = init(Cluster.create(2)))
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                cluster.get(1).executeInternal("DELETE v1 FROM " + KS_TABLE + " USING TIMESTAMP 0 WHERE k=0 and c=? ", i);
            }
            cluster.forEach(c -> c.flush(KEYSPACE));
            ...
            for (int i = 0; i < 10; i++)
            {
                cluster.coordinator(1).execute("INSERT INTO " + KS_TABLE + " (k, c, v2) VALUES (0, ?, ?) USING TIMESTAMP 1",
                                               ConsistencyLevel.ALL, i, i);
            }
            cluster.forEach(c -> c.flush(KEYSPACE));
            ...
            for (int i = 0; i < 10; i++)
            {
                cluster.get(2).executeInternal("INSERT INTO " + KS_TABLE + " (k, c, v2) VALUES (0, ?, ?) USING TIMESTAMP 2", i, i * 2);
            }
    ...
    public void testSnapshottingOnInconsistency() throws Throwable
    {
        try (Cluster cluster = init(Cluster.create(2)))
        {
            ...
            cluster.get(1).runOnInstance(() -> assertSnapshotNotPresent(snapshotName));
            cluster.get(2).runOnInstance(() -> assertSnapshotNotPresent(snapshotName));
            ...
            cluster.get(1).runOnInstance(() -> assertSnapshotPresent(snapshotName));
            cluster.get(2).runOnInstance(() -> assertSnapshotPresent(snapshotName));
        }
    }
    ...
    {
        String exclusions = KEYSPACE + ':' + TABLE + ":666f6f";
                                           ...
                                           .start()))
        {
            ...
            setupSchema(cluster, "CREATE TABLE " + KS_TABLE + " (k INT, c TEXT, v INT, PRIMARY KEY (k,c))");
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 10; j++)
                {
            ...
            cluster.get(1).executeInternal("INSERT INTO " + KS_TABLE + " (k, c, v) VALUES (0, ?, ?)", "foo6", 66);
            cluster.forEach(c -> c.flush(KEYSPACE));
            cluster.forEach(i -> i.runOnInstance(markAllRepaired()));
            cluster.forEach(i -> i.runOnInstance(assertRepaired()));
            ...
            cluster.get(1).executeInternal("INSERT INTO " + KS_TABLE + " (k, c, v) VALUES (0, ?, ?)", "foo6", 666);
            ...
            cluster.get(1).executeInternal("INSERT INTO " + KS_TABLE + " (k, c, v) VALUES (0, ?, ?)", "foo6", 6666);
            cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE + " WHERE k=0", ConsistencyLevel.ALL);
            ccAfter = getConfirmedInconsistencies(cluster.get(1));
            Assert.assertEquals("confirmed count should not increase when exclusions are enabled", ccBefore + 2, ccAfter);

            cluster.get(1).executeInternal("INSERT INTO " + KS_TABLE + " (k, c, v) VALUES (0, ?, ?)", "foo6", 66666);
            cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE, ConsistencyLevel.ALL);
            ccAfter = getConfirmedInconsistencies(cluster.get(1));
            Assert.assertEquals("confirmed count should not increase when exclusions are enabled", ccBefore + 2, ccAfter);
        }
    ...
    public static class BBHelper
    {
                                                               ...
                                                               @SuperCall Callable<UnfilteredPartitionIterator> zuperCall)
        {
            try
            {
                if (cfs.metadata.cfName.equals(TABLE))
                {
    ...
    static IIsolatedExecutor.SerializableRunnable assertNotRepaired()
    {
        return assertRepairedStatus(false);
    }

    static IIsolatedExecutor.SerializableRunnable assertRepaired()
    {
        return assertRepairedStatus(true);
    }

    private static IIsolatedExecutor.SerializableRunnable assertRepairedStatus(boolean repaired)
    {
        return () ->
                ...
                .forEach(reader ->
                         {
                             ...
                             try
                             {
                                 ...
                                 if (repaired && stats.repairedAt == ActiveRepairService.UNREPAIRED_SSTABLE)
                                     throw new AssertionError("repaired at is set for sstable: " + reader.descriptor);
                                 else if (!repaired && stats.repairedAt != ActiveRepairService.UNREPAIRED_SSTABLE)
                                     throw new AssertionError("repaired at is not set for sstable: " + reader.descriptor);
                             } catch (IOException e) {
                                 throw new RuntimeException(e);
                             }
                         });
    ...
    static IIsolatedExecutor.SerializableRunnable markAllRepaired()
    {
        return () ->
            Keyspace.open(KEYSPACE)
                    .getColumnFamilyStore(TABLE)
    ...
    private void assertSnapshotPresent(String snapshotName)
    {
    ...
    private void assertSnapshotNotPresent(String snapshotName)
    {
        ColumnFamilyStore cfs = Keyspace.open(KEYSPACE).getColumnFamilyStore(TABLE);
