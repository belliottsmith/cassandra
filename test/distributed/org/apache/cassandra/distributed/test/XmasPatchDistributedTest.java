...
{
    @Test
    public void testPopulateAndDropTable() throws IOException
    {
                                          ...
                                          .start()))
        {
            ...
            cluster.schemaChange("create table \"" + KEYSPACE.toUpperCase() + "\".\"tbl\" (id int primary key, t int)");
            PreviewRepairTest.insert(cluster.coordinator(1), 0, 100, "tbl");
            PreviewRepairTest.insert(cluster.coordinator(1), 0, 100, "\"tBl\"");
            cluster.forEach((node) -> node.flush(KEYSPACE));
            cluster.get(1).nodetoolResult("repair", "-full", KEYSPACE);
            cluster.get(1).nodetoolResult("repair", "-full", KEYSPACE.toUpperCase());
