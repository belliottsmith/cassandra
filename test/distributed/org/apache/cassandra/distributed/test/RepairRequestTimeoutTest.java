...
import mme.cassandraclient.shaded.com.google.common.util.concurrent.Uninterruptibles;
...
public class RepairRequestTimeoutTest extends TestBaseImpl
{
    ...
    public static void setup() throws IOException
    {
        CLUSTER = init(Cluster.build(3)
                              .withConfig(config -> config.with(GOSSIP, NETWORK).set("repair_message_timeout_in_ms", timeoutMillis))
                              .start());
    ...
    public void testLostMessageHelper(Verb verb)
    {
        ...
        CLUSTER.get(1).runOnInstance(() -> {
            ...
            Uninterruptibles.sleepUninterruptibly((timeoutMillis + expirationInterval) * 2, MILLISECONDS);
        });
