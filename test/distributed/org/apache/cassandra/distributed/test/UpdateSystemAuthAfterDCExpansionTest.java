...
public class UpdateSystemAuthAfterDCExpansionTest extends TestBaseImpl
{
    ...
    static void assertRolePresent(IInstance instance)
    {
        assertRows(instance.executeInternal(String.format("SELECT role FROM %s.%s WHERE role = ?",
                                                         SchemaConstants.AUTH_KEYSPACE_NAME, ROLES), username),
                  row(username));
    }
    ...
    static void assertQueryThrowsConfigurationException(Cluster cluster, String query)
    {
        cluster.forEach(instance -> {
            try
            {
                instance.executeInternal(query); // No need to use cluster.schemaChange as expecting a failure
                                          ...
                                          String beforeDecommissionedDatacenters, String afterDecommissionedDatacenters) throws Throwable
    {
        ...
        {
            logger.info("Starting cluster with single node in dc1");
            cluster.startup();

            logger.info("Creating test role"); // currently no way to set authenticated user for coordinator
            ...
            logger.info("Try changing NTS too early, before a node from the DC has joined");
            assertQueryThrowsConfigurationException(cluster, alterKeyspaceStatement("'dc1': '1', 'dc2': '1'"));

            logger.info("Altering '{}' keyspace to use NTS with {}", SchemaConstants.AUTH_KEYSPACE_NAME, initialDatacenters);

            cluster.schemaChangeIgnoringStoppedInstances(alterKeyspaceStatement(initialDatacenters));

            logger.info("Bootstrapping second node in dc2");
            IInstanceConfig config = cluster.newInstanceConfig();
            config.set("auto_bootstrap", true);
            cluster.bootstrap(config).startup();

            // Check still on node1, but not made it to node2 yet
            assertRolePresent(cluster.get(1));
            assertRoleAbsent(cluster.get(2));

            // Update options to make sure a replica is in the remote DC
            logger.info("Altering '{}' keyspace to use NTS with dc1 & dc2", SchemaConstants.AUTH_KEYSPACE_NAME);
            ...
            logger.info("Repair system_auth to make sure role is replicated everywhere");
            cluster.get(1).runOnInstance(() -> {
                try
                {
            ...
            logger.info("Check the role is now replicated as expected after repairing");
            assertRolePresent(cluster.get(1));
            assertRolePresent(cluster.get(2));

            // Make sure we cannot remove either of the active datacenters
            logger.info("Verify that neither active datacenter can be ALTER KEYSPACEd away");
