...
public class UpgradeTest extends DistributedTestBase
{
    ...
    public void upgradeTest() throws IOException
    {
        ...
        final Versions.Major[][] tests = new Versions.Major[][] {
            new Versions.Major[] { Versions.Major.v22, Versions.Major.v30, Versions.Major.v3X },
            new Versions.Major[] { Versions.Major.v30, Versions.Major.v3X, Versions.Major.v4 }
        };
        for (Versions.Major[] test : tests)
        {
            try (MultiVersionCluster cluster = init(MultiVersionCluster.create(3, versions.getLatest(test[0]))))
            {
                ...
                for (int m = 1 ; m < test.length ; ++m)
                {
                    for (int i = 1 ; i <= 3 ; ++i)
                    {
                        ...
                        cluster.get(i).setVersion(versions.getLatest(test[m]));
