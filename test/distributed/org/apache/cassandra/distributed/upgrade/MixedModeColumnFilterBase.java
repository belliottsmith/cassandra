...
public class MixedModeColumnFilterBase
{
    ...
    static final boolean SIMULATE_UPGRADE_VERSION_OVERRIDE_SKEW = false; // do not want 4.0 to think it is upgrading from 3.0
    ...
    void reportException(IUpgradeableInstance instance, Runnable thing)
    {
        try
        {
            thing.run();
        ...
        {
            logger.error("Instance {} running {} threw exception", instance.config().num(), instance.getReleaseVersionString(), tr);
            failures++;
        }
    }
    ...
    void validate16686_1(IUpgradeableInstance instance)
    {
    ...
    void validate16686_2(IUpgradeableInstance instance)
    {
    ...
    public void run(final List<String> stringVersions) throws Throwable
    {
        ...
        List<Versions.Version> upgradeOrder = stringVersions.stream()
                                                      .map(v -> {
                                                          try
                                                          {
                                                              Versions.Version dv = dtestVersions.get(v);
                                                              assumeTrue(dv != null);
                                                              return dv;
                                                          }
        ...
        java.util.function.Consumer<IInstanceConfig> configUpdater = config -> config.with(Feature.NETWORK, Feature.GOSSIP)
                                                                               .set("read_request_timeout_in_ms", 30000L)
    ...
    {
        logger.info("Shutting down instances {} to upgrade to {}", instanceIdsArray, upgradeTo.version);
        List<Future<Void>> shutdownFutures = Arrays.stream(instanceIdsArray)
                                                   .mapToObj(i -> cluster.get(i).shutdown(true))
                                                   .collect(Collectors.toList());
        FBUtilities.waitOnFutures(shutdownFutures, 2, TimeUnit.MINUTES);
        ...
        Arrays.stream(instanceIdsArray).forEach(instanceId -> {
            ...
            logger.info("Starting instanceId {}", instanceId);
            instance.startup();
            logger.info("Started instanceId {}", instanceId);
        });

        // Most reliable way to determine if messaging service is ready for queries is to execute one
        Awaitility.await()
                  ...
                  .until(() -> {
                      ...
                      try
                      {
                          cluster.forEach(instance -> {
                              // Simple health check query not expected to be impacted by CF bugs.
                              instance.coordinator().execute(
                              "SELECT pk,ck,v1 FROM cftest.tbl_s1v1 WHERE pk = 'CASSANDRA-17601-singlestatic' AND ck=1",
                              ConsistencyLevel.ALL);
                          });
                          return true;
                      }
                      ...
                      {
                          logger.info("Upgraded instances not successfully executing queries yet - {}",
                                      tr.getMessage());
                          return false;
                      }
                  });
        logger.info("All instances are coordinating queries, proceeding");
    }
