...
public class CompactionAllocationTest
{
    ...
    private static void runQuery(ReadQuery query, CFMetaData metadata)
    {
        ReadCommand command = Iterables.getOnlyElement(((SinglePartitionReadCommand.Group) query).commands);
        try (ReadOrderGroup opGroup = query.startOrderGroup();
    ...
    private static void testTinyPartitions(String name, int numSSTable, int sstablePartitions, boolean overlap) throws Throwable
    {
        ...
        SchemaLoader.createKeyspace(ksname, KeyspaceParams.simple(1),
                                    CFMetaData.compile("CREATE TABLE tbl (k INT PRIMARY KEY, v INT)", ksname));
            ...
            public void setup()
            {
                ...
                for (int f = 0; f < numSSTable; f++)
                {
                    for (int p = 0; p < sstablePartitions; p++)
                    {
                        ...
                        if (!overlap || f == 0)
                        {
                            QueryOptions options = QueryProcessor.makeInternalOptions(prepared, new Object[]{f});
                            ReadQuery query = select.getQuery(options, now);
                            reads.add(() -> runQuery(query, cfs.metadata));
                        }
                    }
            ...
            public void setup()
            {
                ...
                for (int f = 0; f < numSSTable; f++)
                {
                    for (int p = 0; p < sstablePartitions; p++)
                    {
                        ...
                        if (!overlap || f == 0)
                        {
                            QueryOptions options = QueryProcessor.makeInternalOptions(prepared, new Object[]{key});
                            ReadQuery query = select.getQuery(options, now);
                            reads.add(() -> runQuery(query, cfs.metadata));
                        }
                    }
