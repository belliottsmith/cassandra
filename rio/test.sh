...
diff --ignore-matching-lines='groupId="com.apple.cie.db.cassandra"\|groupId="org.apache.cassandra"\|default="jar" name="cie-cassandra"\|default="jar" name="apache-cassandra"\|Dcie-cassandra.sizeTieredDefault=\|cie-cassandra.disable_schema_drop_log=' rio-build.xml build.xml
ret=$?
if [ ${ret} -ne 0 ]; then
